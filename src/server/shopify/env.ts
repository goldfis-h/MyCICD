...
const CustomerEnvSchema = v.object({
  SHOPIFY_CUSTOMER_ACCOUNT_CLIENT_ID: v.pipe(
    ...
    v.minLength(
      1,
      'SHOPIFY_CUSTOMER_ACCOUNT_CLIENT_ID is required for customer accounts',
    ),
  ),
  SHOPIFY_CUSTOMER_ACCOUNT_SHOP_ID: v.pipe(
    ...
    v.minLength(
      1,
      'SHOPIFY_CUSTOMER_ACCOUNT_SHOP_ID is required for customer accounts',
    ),
  ),
...
export function getCustomerEnv() {
  ...
  cachedCustomer = v.parse(CustomerEnvSchema, {
    SHOPIFY_CUSTOMER_ACCOUNT_CLIENT_ID: readEnv(
      'SHOPIFY_CUSTOMER_ACCOUNT_CLIENT_ID',
    ),
    SHOPIFY_CUSTOMER_ACCOUNT_SHOP_ID: readEnv(
