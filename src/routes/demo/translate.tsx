...
function TranslateDemo() {
  ...
  return (
    <main className="demo-page" style={{ padding: '2rem', maxWidth: '640px', margin: '0 auto' }}>
      <h1 style={{ fontSize: '1.5rem', fontWeight: 700, marginBottom: '1rem' }}>
        Dynamic Translation Demo
      </h1>

      <p style={{ marginBottom: '1.5rem', color: '#666' }}>
        This page demonstrates calling DeepSeek API to translate messages dynamically.
      </p>
            ...
            <tr key={msg.key} style={{ borderBottom: '1px solid #eee' }}>
              ...
              <td style={{ padding: '0.5rem' }}>
                {translations[msg.key] ? (
                  <span style={{ color: '#059669' }}>{translations[msg.key]}</span>
                ) : (
      ...
      {Object.keys(translations).length > 0 && (
        ...
        >
          ✓ {Object.keys(translations).length} messages translated. Results are cached in memory.
          Click again to return cached results immediately.
        </div>
      )}
    </main>
  )
